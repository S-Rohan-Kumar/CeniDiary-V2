...
const getMovieStats = asyncHandler(async (req, res) => {
  const { movieId } = req.params; // This is the tmdbId from the URL

  const movie = await Movie.findOne({ tmdbId: Number(movieId) });
  if (!movie) {
    // If movie isn't in DB yet, stats are all zero
  ...
  // Constructing the final figures
  const counts = {
  ...
  return res
    .status(200)
    .json(new APIResponse(200, { counts, totalVotes }, "Movie stats fetched"));
});
